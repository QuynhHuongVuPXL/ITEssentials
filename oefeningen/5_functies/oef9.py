...
def vliegtuig_kosten(stad):
    ...
    # kan ook return 0 dan heb je geen vliegtuig kosten


def huurauto_kosten(aantal_dagen):
    huurauto_kost_per_dag = 40

    totale_huurauto_kost = huurauto_kost_per_dag * aantal_dagen

    if aantal_dagen > 7:
        totale_huurauto_kost -= 50
    elif aantal_dagen > 3:
        totale_huurauto_kost -= 20

    return totale_huurauto_kost


def reis_kosten(stad, aantal_dagen):
    totaal_hotel_kosten = hotel_kosten(aantal_dagen)
    ...
    totaal_huurauto_kosten = huurauto_kosten(aantal_dagen)

    totale_prijs = totaal_hotel_kosten + totaal_vliegtuig_kosten + totaal_huurauto_kosten

    return totale_prijs

def main():
    reisbestemming = str(input("Geef een reisbestemming in (Barcelona, Rome, Berlijn of Oslo): "))
    dagen_verblijf = int(input("Geef het aantal dagen dat je in deze stad zal verblijven: "))

    kostprijs = reis_kosten(reisbestemming, dagen_verblijf)
