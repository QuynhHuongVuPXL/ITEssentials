...
def bereken_korting_mutualiteit(type_wagen, afstand_in_km, lid_mutualiteit):
    vaste_som = 0
    if lid_mutualiteit == "J":
        if type_wagen == "R":
            vaste_som += 15
            if afstand_in_km > 10:
                vaste_som += (afstand_in_km - 10) * 1.5
        else:
            vaste_som += 10
            if afstand_in_km > 10:
...
def main():
    ...
    while naam != "/":
        ...
        korting_mutualiteit = bereken_korting_mutualiteit(type_wagen, afstand_in_km, lid_mutualiteit)
